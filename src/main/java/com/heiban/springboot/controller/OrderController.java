...
import com.heiban.springboot.utils.UniqueOrder;
...
public class OrderController {
    ...
    public void paymethod(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        String nowtime = uniqueOrder.getUniqueOrder();
//                String.valueOf(System.currentTimeMillis());
    ...
    //网页订单发起方法
    @PostMapping("/pagepaymethod")
    public void pagePaymethod(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
    ...
    public String pcRefund(Long id) {
        AlipayClient client = new DefaultAlipayClient(AlipayConfig.URL, AlipayConfig.APPID, AlipayConfig.RSA_PRIVATE_KEY, AlipayConfig.FORMAT, AlipayConfig.CHARSET, AlipayConfig.ALIPAY_PUBLIC_KEY, AlipayConfig.SIGNTYPE);
        AlipayTradeRefundRequest refundrequest = new AlipayTradeRefundRequest();
        AlipayTradeRefundModel model = new AlipayTradeRefundModel();
        Order order = orderRepository.getOne(id);
        AlipayTradeFastpayRefundQueryResponse queryResponse = RefundQuery(id);
        if( order.getPlay()==3){
            return "redirect:/information.html";
        }
        String trade_no = order.getTrade_no();//支付宝交易号
        String out_trade_no = order.getOut_trade_no();//商户交易号
        String orderMoney = order.getMoney();//退款金额
//        model.setOutTradeNo(trade_no);
        model.setOutTradeNo(out_trade_no);
        model.setRefundAmount(orderMoney);
        model.setRefundReason("正常退款");
        refundrequest.setBizModel(model);
        AlipayTradeRefundResponse alipayTradeRefundResponse = null;
        try {
            alipayTradeRefundResponse = client.execute(refundrequest);
            if (alipayTradeRefundResponse.isSuccess()||queryResponse.getMsg().equals("Success")||queryResponse.getCode().equals("10000")) {//退款成功
                System.out.println("退款成功！请注意查看");
                System.out.println(queryResponse.getMsg());
    ...
    @ResponseBody
    public AlipayTradeFastpayRefundQueryResponse RefundQuery(Long id) {
        AlipayClient client = new DefaultAlipayClient(AlipayConfig.URL, AlipayConfig.APPID, AlipayConfig.RSA_PRIVATE_KEY, AlipayConfig.FORMAT, AlipayConfig.CHARSET, AlipayConfig.ALIPAY_PUBLIC_KEY, AlipayConfig.SIGNTYPE);
        AlipayTradeFastpayRefundQueryRequest tradeFastpayRefundQueryRequest = new AlipayTradeFastpayRefundQueryRequest();
        AlipayTradeRefundApplyModel applyModel = new AlipayTradeRefundApplyModel();
        Order order = orderRepository.getOne(id);
        applyModel.setTradeNo(order.getTrade_no());
        applyModel.setOutRequestNo(order.getOut_trade_no());
        tradeFastpayRefundQueryRequest.setBizModel(applyModel);
        try {
            AlipayTradeFastpayRefundQueryResponse queryResponse = client.execute(tradeFastpayRefundQueryRequest);
